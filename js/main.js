  ...
  btnMore.addEventListener('click', function () {
    ...
    window.addEventListener('scroll', function () {
        if (!isLoading && window.innerHeight + window.scrollY >= document.body.offsetHeight - 500) {
            ...
            loading.style.display = 'block'; // 로딩 이미지 보이기
            setTimeout(function () {
                loadImages();
                loading.style.display = 'none'; // 로딩 이미지 숨기기
  ...
  function loadImages() {
      for (var i = 0; i < imagesPerLoad; i++) {
          if (imgIndex > maxImage) {
              imgIndex = 1; // 이미지 인덱스 초기화
          }
          var imgElement = document.createElement('li');
          imgElement.innerHTML = '<img src="./images/img_' + ("0" + imgIndex).slice(-2) + '.jpg" alt="">';
